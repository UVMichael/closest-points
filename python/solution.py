...
class Solution:
    ...
    def trainingPenalty(self):
        ...
        if not self.valid():
            for city in self.instance.cities:
                minDist = 100000000
                for tower in self.towers:
                    dist = city.distance_obj(tower).value
                    if (dist < minDist):
                        minDist = dist
                if (minDist > self.instance.coverage_radius):
                    penalty += 250 * minDist + 10000
        # End Thomas Addition
    ...
    def penalty_local(changed_tower, self):
        """Computes the penalty for this solution."""
        penalty = 0
        range = 20
        subset_close = []
        for tower in self.towers:
            if (tower == changed_tower):
                continue
            if Point.distance_obj(changed_tower, second) <= range:
                    subset_close.append(tower)       
        for fidx, first in enumerate(subset_close):
            num_overlaps = 0
            for sidx, second in enumerate(subset_close):
                if fidx == sidx:
                    continue
                if Point.distance_obj(first, second) <= self.instance.penalty_radius:
                    num_overlaps += 1
